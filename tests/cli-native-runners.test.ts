...
import {
  ...
  runSwitchboardRelayDeploymentStatus,
  runSwitchboardRelayDeployments,
  runSwitchboardRelayDiff,
  runSwitchboardRelayInspect,
...
describe("native command runners", () => {
  ...
  it("exports a shared relay deployments runner for local history inspection", async () => {
    const workDir = await mkdtemp(path.join(tmpdir(), "switchboard-native-relay-deployments-"));
    try {
      await mkdir(path.join(workDir, ".switchboard", "relays"), { recursive: true });
      await writeFile(
        path.join(workDir, ".switchboard", "relays", "relay-d.history.json"),
        JSON.stringify({
          relayId: "relay-d",
          entries: [
            {
      ...
      await withWorkingDirectory(workDir, async () => {
        const textOutputs = [
          await captureConsole(() => runSwitchboardRelayDeployments(["relay-d"], runtimeWithoutLatestReport)),
          await captureConsole(() =>
            runSwitchboardRelayDeployments(["relay", "deployments", "relay-d"], runtimeWithoutLatestReport)
          )
        ];
        assert.equal(textOutputs[0], textOutputs[1]);
        assert.match(textOutputs[0], /relay-d deployment history \(1 entry\)/u);
        assert.match(textOutputs[0], /2026-05-02T12:00:00.000Z  success/u);
        assert.match(textOutputs[0], /note        : candidate rollout/u);

        const jsonOutputs = [
          JSON.parse(await captureConsole(() => runSwitchboardRelayDeployments(["relay-d", "--json"], runtimeWithoutLatestReport))),
          JSON.parse(await captureConsole(() =>
            runSwitchboardRelayDeployments(["relay", "deployments", "relay-d", "--json"], runtimeWithoutLatestReport)
          ))
        ] as Array<{ relayId: string; entries: Array<{ outcome: string; notes?: string }> }>;
        assert.deepEqual(jsonOutputs[0], jsonOutputs[1]);
        assert.equal(jsonOutputs[0].relayId, "relay-d");
        assert.equal(jsonOutputs[0].entries[0]?.outcome, "success");
        assert.equal(jsonOutputs[0].entries[0]?.notes, "candidate rollout");
      });
    } finally {
      await rm(workDir, { recursive: true, force: true });
    }
  ...
  it("keeps relay deployments empty-history and relay-id errors unchanged", async () => {
    const workDir = await mkdtemp(path.join(tmpdir(), "switchboard-native-relay-deployments-empty-"));
    try {
      await withWorkingDirectory(workDir, async () => {
        const output = await captureConsole(() =>
          runSwitchboardRelayDeployments(["relay", "deployments", "relay-d"], runtimeWithoutLatestReport)
        );
        assert.match(output, /No deployment history for relay-d/u);
        assert.match(output, /\.switchboard\/relays\/relay-d\.history\.json not found/u);

        await assert.rejects(
          runSwitchboardRelayDeployments([], runtimeWithoutLatestReport),
          /Usage: switchboard relay deployments <relay-id>/
        );
        await assert.rejects(
          runSwitchboardRelayDeployments(["BadRelay"], runtimeWithoutLatestReport),
          /Usage: switchboard relay deployments <relay-id>/
        );
      });
    } finally {
  ...
  it("exports a shared relay deployment-status runner for native plugin reuse", async () => {
    const workDir = await mkdtemp(path.join(tmpdir(), "switchboard-native-relay-deployment-status-"));
    try {
      await mkdir(path.join(workDir, "specs"), { recursive: true });
      await mkdir(path.join(workDir, ".switchboard", "relays"), { recursive: true });
      const spec = {
        version: 1,
        relayId: "relay-d",
        ...
        acurast: {
          ...
          network: "mainnet",
          projectName: "relay-d",
          stageDir: "dist/acurast/relay-d",
          maxCostPerExecution: 1,
          includeEnv: []
        }
      };
      const bareSpec = path.join(workDir, "specs", "relay-d-bare.json");
      const prefixedSpec = path.join(workDir, "specs", "relay-d-prefixed.json");
      await writeFile(bareSpec, JSON.stringify(spec), "utf8");
      await writeFile(prefixedSpec, JSON.stringify(spec), "utf8");
      await writeFile(
        path.join(workDir, ".switchboard", "relays", "relay-d.history.json"),
        JSON.stringify({
          relayId: "relay-d",
      ...
      const lifecycleOptions = {
        ...
        spawnPnpm: async (args: string[], env: NodeJS.ProcessEnv, cwd: string) => {
          calls.push({ args: [...args], env, cwd });
          return 0;
        }
      };

      await runSwitchboardRelayDeploymentStatus(
        ["relay-d", "--deployment-id", "51808", "--spec", bareSpec],
        runtimeWithoutLatestReport,
        lifecycleOptions
      );
      await runSwitchboardRelayDeploymentStatus(
        ["relay", "deployment-status", "relay-d", "--spec-file", prefixedSpec],
        runtimeWithoutLatestReport,
        lifecycleOptions
      );

      assert.deepEqual(calls.map((call) => call.args), [
        ["acurast:status-express", "--", "--deployment-id", "51808"],
        ["acurast:status-express", "--", "--deployment-id", "51809"]
      ]);
      assert.deepEqual(calls.map((call) => call.cwd), [workDir, workDir]);
      assert.equal(calls[0].env.ACURAST_MAINNET_SEED, "test-seed");
      assert.equal(calls[0].env.ACURAST_NETWORK, "mainnet");
      assert.equal(calls[0].env.ACURAST_PROJECT_NAME, "relay-d");
  ...
  it("exports a shared relay inspect runner for native plugin reuse", async () => {
    const workDir = await mkdtemp(path.join(tmpdir(), "switchboard-native-relay-inspect-"));
    try {
      await mkdir(path.join(workDir, "specs"), { recursive: true });
      await mkdir(path.join(workDir, ".switchboard", "relays"), { recursive: true });
      const spec = {
      ...
      await runSwitchboardRelayInspect(
        ["relay-d", "--deployment-id", "51808", "--spec", bareSpec, "--watch"],
        runtimeWithoutLatestReport,
        lifecycleOptions
      );
      await runSwitchboardRelayInspect(
        ["relay", "inspect", "relay-d", "--spec-file", prefixedSpec, "--events"],
        runtimeWithoutLatestReport,
        lifecycleOptions
      );

      assert.deepEqual(calls.map((call) => call.args), [
        ["acurast:inspect-express", "--", "--deployment-id", "51808", "--watch"],
        ["acurast:inspect-express", "--", "--deployment-id", "51809", "--events"]
      ]);
      assert.deepEqual(calls.map((call) => call.cwd), [workDir, workDir]);
      assert.equal(calls[0].env.ACURAST_MAINNET_SEED, "test-seed");
      assert.equal(calls[0].env.ACURAST_NETWORK, "mainnet");
  ...
  it("keeps relay inspect argument and helper errors unchanged", async () => {
    const workDir = await mkdtemp(path.join(tmpdir(), "switchboard-native-relay-inspect-errors-"));
    try {
      await mkdir(path.join(workDir, "relays"), { recursive: true });
      const acurastSpec = {
        version: 1,
      ...
      const acurastSpecFile = path.join(workDir, "relays", "relay-d.json");
      const mismatchedSpecFile = path.join(workDir, "relays", "relay-c.json");
      const bootstrapSpecFile = path.join(workDir, "relays", "relay-a.json");
      await writeFile(acurastSpecFile, JSON.stringify(acurastSpec), "utf8");
      await writeFile(mismatchedSpecFile, JSON.stringify({ ...acurastSpec, relayId: "relay-c" }), "utf8");
      await writeFile(
        bootstrapSpecFile,
        JSON.stringify({
          version: 1,
      ...
      await assert.rejects(
        runSwitchboardRelayInspect([], runtimeWithoutLatestReport, lifecycleOptions),
        /Usage: switchboard relay inspect <relay-id> \[--deployment-id <n>\]/
      );
      await assert.rejects(
        runSwitchboardRelayInspect(["BadRelay"], runtimeWithoutLatestReport, lifecycleOptions),
        /Usage: switchboard relay inspect <relay-id> \[--deployment-id <n>\]/
      );
      await assert.rejects(
        runSwitchboardRelayInspect(
          ["relay-d", "--spec", acurastSpecFile, "--deployment-id", "bad"],
          runtimeWithoutLatestReport,
          lifecycleOptions
        ),
        /--deployment-id must be a numeric Acurast job sequence/
      );
      await assert.rejects(
        runSwitchboardRelayInspect(["missing", "--deployment-id", "51808"], runtimeWithoutLatestReport, lifecycleOptions),
        /Spec .*relays\/missing\.json not found/
      );
      await assert.rejects(
        runSwitchboardRelayInspect(
          ["relay-a", "--spec", bootstrapSpecFile, "--deployment-id", "51808"],
          runtimeWithoutLatestReport,
          lifecycleOptions
        ),
        /relay inspect is acurast-target only/
      );
      await assert.rejects(
        runSwitchboardRelayInspect(
          ["relay-d", "--spec", mismatchedSpecFile, "--deployment-id", "51808"],
          runtimeWithoutLatestReport,
          lifecycleOptions
        ),
        /declares relayId=relay-c, but command was invoked for relay-d/
      ...
      await assert.rejects(
        runSwitchboardRelayInspect(
          ["relay-d", "--spec", acurastSpecFile, "--deployment-id", "51808"],
          runtimeWithoutLatestReport,
          {
            ...lifecycleOptions,
            spawnPnpm: async () => 7
        ...
        /acurast:inspect-express exited with code 7/
      );
    } finally {
      await rm(workDir, { recursive: true, force: true });
    }
  ...
  it("keeps relay deployment-status argument and helper errors unchanged", async () => {
    const workDir = await mkdtemp(path.join(tmpdir(), "switchboard-native-relay-deployment-status-errors-"));
    try {
      await mkdir(path.join(workDir, "relays"), { recursive: true });
      const acurastSpec = {
        version: 1,
      ...
      await assert.rejects(
        runSwitchboardRelayDeploymentStatus([], runtimeWithoutLatestReport, lifecycleOptions),
        /Usage: switchboard relay status <relay-id> \[--deployment-id <n>\]/
      );
      await assert.rejects(
        runSwitchboardRelayDeploymentStatus(["BadRelay"], runtimeWithoutLatestReport, lifecycleOptions),
        /Usage: switchboard relay status <relay-id> \[--deployment-id <n>\]/
      );
      await assert.rejects(
        runSwitchboardRelayDeploymentStatus(
      ...
      await assert.rejects(
        runSwitchboardRelayDeploymentStatus(["missing", "--deployment-id", "51808"], runtimeWithoutLatestReport, lifecycleOptions),
        /Spec .*relays\/missing\.json not found/
      );
      await assert.rejects(
        runSwitchboardRelayDeploymentStatus(
        ...
        /relay status is acurast-target only/
      );
      await assert.rejects(
        runSwitchboardRelayDeploymentStatus(
          ["relay-d", "--spec", acurastSpecFile, "--deployment-id", "51808"],
        ...
        /acurast:status-express exited with code 7/
      );
    } finally {
      await rm(workDir, { recursive: true, force: true });
    }
